...
// ---------------------------------------------------------------------------
// 阶段2: XCity（xcity.jp）— 日文有码源（有年龄墙）。
// 参考 amane（github.com/sqzw-x/amane）crawlers/sites/xcity.py 的同源设计重写。
// 年龄墙: amane 方案携带 cookie age_check_done=1；基类 GetHtmlAsync 不支持附加
//         Cookie 头，因此在搜索/详情前先 GET 一次站点首页，依赖 MultiTierFetcher
//         的 Cookie 罐把首页下发的年龄确认 cookie 带到后续请求（预热失败不阻断）。
// 搜索: GET {base}/search/?keyword={番号}（置信度低：完整参数集需联网冒烟核实）；
//       详情: /avod/detail/...，字段表按 th 标签扫描
//       （発売日/収録時間/出演者/監督/メーカー/レーベル/シリーズ/ジャンル）。
// ---------------------------------------------------------------------------

public sealed class XcitySource : HtmlScrapeSourceBase
{
    private const string BaseUrl = "http://www.xcity.jp";

    public XcitySource(IResilientFetcher fetcher, ILogger? logger = null) : base(fetcher, logger) { }

    public override string Id => "xcity";
    public override string DisplayName => "XCity";
    ...
    protected override async Task<VideoScrapeMetadata?> FetchCoreAsync(VideoSourceRequest request, CancellationToken ct)
    {
        ...
        foreach (var a in doc.QuerySelectorAll("a[href*='/avod/detail/']")) // 置信度低：详情链接路径模式
        {
        ...
        // 兜底：唯一命中时站点可能从搜索页直接跳到详情（标题区已含番号）
        ...
        var title = (root.QuerySelector("#title h1")?.TextContent
                     ?? root.QuerySelector("h1")?.TextContent ?? "").Trim();
        // 置信度低：封面优先 jacket/main 图，回退 og:image
        var cover =
            root.QuerySelector("img[src*='jacket']")?.GetAttribute("src")
            ?? root.QuerySelector("img[src*='/main/']")?.GetAttribute("src")
        ...
        var actorCell = ElementByLabel(root, "出演者", "出演");
        if (actorCell is not null) AddLinksOrSplit(actorCell, metadata.Actors);
        var tagCell = ElementByLabel(root, "ジャンル", "タグ");
    ...
    /// 由抓取栈的 Cookie 罐持有年龄确认 cookie（amane: age_check_done=1）供后续使用。
    /// </summary>
