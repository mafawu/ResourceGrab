...
### P0-2　`ScrapeReportService` 未注册 DI + 三实例互写覆盖
...
### P1-1　`MainWindow.xaml.cs` 上帝对象（1172 行）
...
### P1-4　`VideoFileCard` 已标 `[Obsolete]` 仍在用
...
### P1-5　`VideoScrapeService` 内建 legacy 源实例与 DI 适配器并存
...
### P1-6　`System.Text.Encoding.CodePages` 的 NU1510 警告
...
| 阶段 | 范围 | 预估工作量 | 验收 |
| --- | --- | --- | --- |
| **阶段 0** | P0-1 ~ P0-4 | 1-2 天 | 代理在线预览可用；报告面板有数据；警告清零 |
| **阶段 1** | P1-4、P1-6、P3-1 | 1 天 | 死代码删除，slnx 含测试 |
| **阶段 2** | P1-3、P1-5 | 2-3 天 | 单一搜索实现，legacy 走 DI |
| **阶段 3** | P1-2（导航精简） | 2 天 | 路由只有一套返回栈 |
| **阶段 4** | P1-1（MainWindow 拆分） | 5-7 天 | MainWindow < 600 行，按视图分步迁移 |
| **阶段 5** | P2-2、P3-2 | 穿插进行 | Save 全原子写；6 个纯逻辑模块有测试 |
